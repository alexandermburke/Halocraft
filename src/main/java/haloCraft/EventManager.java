...
public class EventManager implements IWorldGenerator {
	...
	private void generateEnd(World world, Random random, int x, int z) {
		...
		(new WorldGenMinable(HaloCraft.Moissanite_Ore, 45, Blocks.end_stone))
				.generate(world, random, Xcoord, Ycoord, Zcoord);
	}

	private void generateSurface(World world, Random random, int x, int z) {
		this.addOreSpawn(HaloCraft.Titanium_Ore, world, random, x, z, 16, 16,
				3 + random.nextInt(4), 6, 5, 30);
		this.addOreSpawn(HaloCraft.Aluminum_Ore, world, random, x, z, 16, 16,
				2 + random.nextInt(3), 8, 1, 63);
		this.addOreSpawn(HaloCraft.Vanadium_Ore, world, random, x, z, 16, 16,
				4 + random.nextInt(2), 10, 1, 40);
		this.addOreSpawn(HaloCraft.Lithium_Ore, world, random, x, z, 16, 16,
				2 + random.nextInt(4), 9, 1, 40);
		this.addOreSpawn(HaloCraft.Niobium_Ore, world, random, x, z, 16, 16,
				3 + random.nextInt(3), 9, 1, 45);
