...
public class HaloArmorDyes implements IRecipe
{
    public Item armorPiece;
    public Item dye;
    public boolean matches(InventoryCrafting inventory, World world)
    {
        ...
        for (int i = 0; i < inventory.getSizeInventory(); ++i)
        {
            ...
            if (itemstack1 != null)
            {
                if (itemstack1.getItem() instanceof MjolnirMarkV)
                {
                	MjolnirMarkV itemarmor = (MjolnirMarkV)itemstack1.getItem();

                    if (itemarmor.getArmorMaterial() != HaloCraft.Mjolnir_MarkV || itemstack != null)
                    {
                        return false;
                    }

                    itemstack = itemstack1;
                    this.armorPiece = itemstack1.getItem();
                }
                else
                {
    ...
    public ItemStack getCraftingResult(InventoryCrafting inventory)
    {
        ...
        for (k = 0; k < inventory.getSizeInventory(); ++k)
        {
            ...
            if (itemstack1 != null)
            {
                if (itemstack1.getItem() instanceof MjolnirMarkV)
                {
                    itemarmor = (MjolnirMarkV)itemstack1.getItem();
